...
class ToolRegistry {
  ...
  getToolsForUser(userRole = 'user') {
    ...
    const filteredTools = Array.from(this.tools.values()).filter(tool => {
    ...
    // CRITICAL: Log which tools are being offered to Gemini
    logger.info('Tools offered to Gemini after RBAC filtering', {
      userRole,
...
class ToolLoader {
  ...
  async loadFromDirectory(directory, source = 'unknown') {
    try {
      ...
      // Load .js files (excluding test files and index.js)
      ...
      for (const file of jsFiles) {
        await this.loadToolFile(path.join(directory, file.name), source);
      }
      ...
      for (const subdir of subdirs) {
        await this.loadFromDirectory(
          path.join(directory, subdir.name),
          `${source}/${subdir.name}`
        );
      }
  ...
  async loadToolFile(filePath, source = 'unknown') {
    try {
      ...
      // Check platform integration flags from Firestore
      const db = getFirestore();
      let ENABLE_BITRIX24 = false;
      let ENABLE_ASANA = false;

      try {
        const bitrixDoc = await db.collection('platform-settings').doc('bitrix24').get();
        ENABLE_BITRIX24 = bitrixDoc.exists && bitrixDoc.data().enabled === true;

        const asanaDoc = await db.collection('platform-settings').doc('asana').get();
        ENABLE_ASANA = asanaDoc.exists && asanaDoc.data().enabled === true;
      } catch (error) {
        logger.warn('Could not load platform settings from Firestore', { error: error.message });
      }
